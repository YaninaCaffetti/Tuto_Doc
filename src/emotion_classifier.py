...
def train_and_evaluate_emotion_classifier(config: dict) -> Dict[str, float]:
    ...
    # 2) Split (NO estratificado en esta versión)
    train_val_df, test_df = train_test_split(
        df_base, test_size=0.20, random_state=RANDOM_STATE
    )
    ...
    # 6) Pesos de clase (Método simple original)
    ...
    with mlflow.start_run(run_name="train_emotion_classifier_final"):
        # Se duplica el log de parámetros, como en la versión original
